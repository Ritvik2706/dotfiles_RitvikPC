-- Personal keymaps configuration
...
-- Alt key keymaps (configure your terminal to support Alt as Meta)
...
-- Select current hunk in visual mode
...
vim.keymap.set({ "n", "v", "i" }, "<M-R>", function()
  ...
  vim.cmd("wall")
  -- Check if a tmux right pane exists, if it does close it
  local has_panes = vim.fn.system("tmux list-panes | wc -l"):gsub("%s+", "") ~= "1"
  if has_panes then
    vim.fn.system("tmux kill-pane -t :.+")
  end
  -- For Linux, just restart nvim in the current tmux session
  vim.cmd("qall")
...
-- TEMPORARILY DISABLED: All escape sequence mappings to test if they're causing issues
-- vim.keymap.set("i", "jk", "<ESC>", { desc = "[P]Exit insert mode with jk" })

-- DISABLED: Original kj mapping that might be causing conflicts
-- vim.keymap.set("i", "kj", function()
--   -- Only exit if we're actually in insert mode and not in a rapid key sequence
--   if vim.fn.mode() == "i" then
--     vim.cmd("stopinsert")
--     -- Small delay to ensure clean mode transition
...
--     end, 1)
--   end
-- end, { desc = "[P]Exit insert mode with kj" })

-- PROTECTED ESCAPE KEY MAPPINGS (moved from options.lua to fix E5108 error)
-- Add explicit escape key mapping to ensure it always works
...
end, { desc = "Exit insert mode (protected)", noremap = true, silent = true })

-- Add terminal-specific escape sequence handling
-- This catches escape sequences that might be causing the issue
vim.keymap.set("n", "<Esc>", function()
  -- If we're somehow not in normal mode, force it
  if vim.fn.mode() ~= "n" then
    vim.cmd("stopinsert")
  end
  -- Clear any pending operations
  vim.cmd("nohlsearch")
end, { desc = "Force normal mode", noremap = true, silent = true })

-- DIRECT FIX: Override movement keys to prevent accidental insert mode entry
-- These mappings ensure that j, k, h, l in normal mode NEVER enter insert mode
vim.keymap.set("n", "j", function()
  -- Force normal mode and execute movement
  if vim.fn.mode() ~= "n" then
    vim.cmd("stopinsert")
  end
  vim.cmd("normal! j")
end, { desc = "Move down (protected)", noremap = true, silent = true })
...
vim.keymap.set("n", "k", function()
  ...
  vim.cmd("normal! k")
end, { desc = "Move up (protected)", noremap = true, silent = true })
...
vim.keymap.set("n", "h", function()
  ...
  vim.cmd("normal! h")
end, { desc = "Move left (protected)", noremap = true, silent = true })
...
vim.keymap.set("n", "l", function()
  ...
  vim.cmd("normal! l")
end, { desc = "Move right (protected)", noremap = true, silent = true })

-- TEMPORARILY DISABLED: Safeguard mappings for k and j
-- Testing if these are causing conflicts
-- vim.keymap.set("n", "k", function()
...
--     vim.cmd("normal! j")
--   end
-- end, { desc = "Move down (safe)", noremap = true, silent = true })
  ...
  vim.defer_fn(function()
    ...
    upload_to_imgur = function()
      ...
      elseif vim.fn.executable("xclip") == 1 then
        ...
        vim.notify("No clipboard tool found. Install wl-clipboard (Wayland) or xclip (X11)", vim.log.levels.ERROR)
        return
      end
...
-- https://youtu.be/Lzl_0SzbUBo
--
-- Open image under cursor in image viewer
vim.keymap.set("n", "<leader>io", function()
  ...
  if image_path then
    ...
    if string.sub(image_path, 1, 4) == "http" then
      ...
      local current_file_path = vim.fn.expand("%:p:h")
      local absolute_image_path = current_file_path .. "/" .. image_path
      -- OS-specific command to open image
      local command
      -- For Linux, try common image viewers in order of preference
      if vim.fn.executable("feh") == 1 then
        command = "feh " .. vim.fn.shellescape(absolute_image_path) .. " &"
      elseif vim.fn.executable("eog") == 1 then
        command = "eog " .. vim.fn.shellescape(absolute_image_path) .. " &"
      elseif vim.fn.executable("gwenview") == 1 then
        command = "gwenview " .. vim.fn.shellescape(absolute_image_path) .. " &"
      elseif vim.fn.executable("ristretto") == 1 then
        command = "ristretto " .. vim.fn.shellescape(absolute_image_path) .. " &"
      else
        command = "xdg-open " .. vim.fn.shellescape(absolute_image_path) .. " &"
      end
      -- Execute the command
...
end, { desc = "[P]Open image under cursor in image viewer" })
...
-- https://youtu.be/Lzl_0SzbUBo
--
-- Open image under cursor in file manager
--
...
vim.keymap.set("n", "<leader>if", function()
  ...
  if image_path then
    ...
    if string.sub(image_path, 1, 4) == "http" then
      ...
      local command
      -- For Linux, try common file managers
      if vim.fn.executable("thunar") == 1 then
        command = "thunar " .. vim.fn.shellescape(vim.fn.fnamemodify(absolute_image_path, ":h")) .. " &"
      elseif vim.fn.executable("nautilus") == 1 then
        command = "nautilus " .. vim.fn.shellescape(vim.fn.fnamemodify(absolute_image_path, ":h")) .. " &"
      elseif vim.fn.executable("dolphin") == 1 then
        command = "dolphin --select " .. vim.fn.shellescape(absolute_image_path) .. " &"
      elseif vim.fn.executable("nemo") == 1 then
        command = "nemo " .. vim.fn.shellescape(vim.fn.fnamemodify(absolute_image_path, ":h")) .. " &"
      else
        command = "xdg-open " .. vim.fn.shellescape(vim.fn.fnamemodify(absolute_image_path, ":h")) .. " &"
      end
      local success = vim.fn.system(command)
      if success == 0 then
        print("Opened image location in file manager: " .. absolute_image_path)
      else
        print("Failed to open image location in file manager: " .. absolute_image_path)
      end
    end
  else
    print("No image found under the cursor")
...
-- https://youtu.be/Lzl_0SzbUBo
--
-- Delete image file under cursor using trash
vim.keymap.set("n", "<leader>id", function()
  ...
  -- Check available trash utilities for Linux
  local trash_command = nil
  local trash_utility = ""

  -- Linux trash utilities
  if vim.fn.executable("trash-put") == 1 then
    trash_command = { "trash-put", vim.fn.fnameescape(absolute_image_path) }
    trash_utility = "trash-put"
  elseif vim.fn.executable("gio") == 1 then
    trash_command = { "gio", "trash", vim.fn.fnameescape(absolute_image_path) }
    trash_utility = "gio trash"
  elseif vim.fn.executable("trash") == 1 then
    trash_command = { "trash", vim.fn.fnameescape(absolute_image_path) }
    trash_utility = "trash"
  end

  if not trash_command then
    local install_msg = "- In Linux run `sudo pacman -S trash-cli` or equivalent for your distro\n"
    vim.api.nvim_echo({
  ...
  vim.ui.select({ "yes", "no" }, { prompt = "Delete image file? " }, function(choice)
    ...
    if choice == "yes" then
      ...
      if success and vim.fn.filereadable(absolute_image_path) == 1 then
        ...
        -- trash, they'll be gone permanently
        --
        -- Cannot see the popup as the cursor is on top of the image name, so saving
        -- its position, will move it to the top and then move it back
      ...
      elseif success and vim.fn.filereadable(absolute_image_path) == 0 then
        vim.api.nvim_echo({
          { "Image file deleted using " .. trash_utility .. ":\n", "Normal" },
          { absolute_image_path, "Normal" },
        }, false, {})
        -- require("image").clear()
        vim.cmd("edit!")
...
-- Removed YouTube embed processing and related keymaps
...
-- French accent auto-correction
-- Correct the French word under cursor with proper accents
vim.keymap.set("n", "<leader>mfa", function()
  local french_accents = require("config.french-accents")
  french_accents.correct_word_under_cursor()
end, { desc = "[P]French accent correction" })
...
function _G.markdown_foldexpr()
  ...
  if heading then
    ...
    if level >= 1 and level <= 6 then
      -- Handle all heading levels (H1-H6) consistently
      return ">" .. level
    end
  end
  return "="
...
local function set_markdown_folding()
  ...
  -- Enable automatic fold updating to handle edits better
...
-- Refresh folds manually - useful when folds get corrupted after editing
vim.keymap.set("n", "zr", function()
  vim.cmd("normal! zx") -- Refresh fold definitions
  vim.notify("Folds refreshed", vim.log.levels.INFO)
end, { desc = "[P]Refresh fold definitions" })
...
local function update_markdown_toc(heading2, heading3)
  ...
  --     vim.api.nvim_buf_del_lines(bufnr, 0, 1, false)
  --   else
  --     break
  --   end
  -- end
...
-- https://youtu.be/BVyrXsZ_ViA
--
-- General purpose TOC update
vim.keymap.set("n", "<leader>mto", function()
  update_markdown_toc("## Table of contents", "### Contents")
end, { desc = "[P]Update Markdown TOC" })

-- Removed YouTube-specific TOC update mapping
